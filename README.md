...
Google Calendar への予定作成・更新にも対応しており、突発的な作業は FAB から下書きセッションとして開始できます。  
...
- 同期済みイベントなら、メモは Google Calendar の予定詳細説明欄にも追記

4. Toggl 連携
- 記録操作に合わせて Toggl の時間エントリ作成を試行
- 終了時に実行中エントリを停止

5. 記録ダッシュボード
- 進行中イベントのタイマー表示
- 停止ボタンは長押しで終了、ダブルタップで即終了
- FAB からタイトル入力して突発予定の下書きを開始
...
本アプリは Settings 画面で Google サインインを行い、Calendar 書き込み権限を使用します。  
既存アカウントで再ログインする場合は、予定作成・更新の許可を再度付与してください。

### 権限について

- 予定の取得だけでなく、予定の作成・更新にも `calendar.events` を使用します
- メモの追記や FAB から開始した突発予定の確定時に、Google Calendar へ書き込みます
...
4. FAB からタイトルを入力して突発予定の下書きを開始
5. 進行中イベントから記録追加（写真/メモ/音声）
6. 停止時は長押しまたはダブルタップで終了
7. 履歴・詳細画面で記録を確認
...
- Google カレンダーの権限変更後は、設定画面から再ログインが必要です。
